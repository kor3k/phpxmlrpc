 ...
 */
class Wrapper implements Log\LoggerAwareInterface
{
    ...
    public function wrapPhpFunction($callable, $newFuncName = '', $extraOptions = array())
    {
        ...
        if (is_array($callable)) {
            if (count($callable) < 2 || (!is_string($callable[0]) && !is_object($callable[0]))) {
                $this->logger->error('XML-RPC: ' . __METHOD__ . ': syntax for function to be wrapped is wrong');
                return false;
            }
            ...
            if (!$buildIt) {
                $this->logger->error('XML-RPC: ' . __METHOD__ . ': a closure can not be wrapped in generated source code');
                return false;
            }
        ...
        if (!$exists) {
            $this->logger->error('XML-RPC: ' . __METHOD__ . ': function to be wrapped is not defined: ' . $plainFuncName);
            return false;
        }
    ...
    protected function introspectFunction($callable, $plainFuncName)
    {
        ...
        if (is_array($callable)) {
            ...
            if ($func->isPrivate()) {
                $this->logger->error('XML-RPC: ' . __METHOD__ . ': method to be wrapped is private: ' . $plainFuncName);
                return false;
            }
            if ($func->isProtected()) {
                $this->logger->error('XML-RPC: ' . __METHOD__ . ': method to be wrapped is protected: ' . $plainFuncName);
                return false;
            }
            if ($func->isConstructor()) {
                $this->logger->error('XML-RPC: ' . __METHOD__ . ': method to be wrapped is the constructor: ' . $plainFuncName);
                return false;
            }
            if ($func->isDestructor()) {
                $this->logger->error('XML-RPC: ' . __METHOD__ . ': method to be wrapped is the destructor: ' . $plainFuncName);
                return false;
            }
            if ($func->isAbstract()) {
                $this->logger->error('XML-RPC: ' . __METHOD__ . ': method to be wrapped is abstract: ' . $plainFuncName);
                return false;
            }
        ...
        if ($func->isInternal()) {
            ...
            $this->logger->error('XML-RPC: ' . __METHOD__ . ': function to be wrapped is internal: ' . $plainFuncName);
            return false;
        }
    ...
    protected function retrieveMethodSignature($client, $methodName, array $extraOptions = array())
    {
        ...
        if ($response->faultCode()) {
            $this->logger->error('XML-RPC: ' . __METHOD__ . ': could not retrieve method signature from remote server for method ' . $methodName);
            return false;
        }
        ...
        if (!is_array($mSig) || count($mSig) <= $sigNum) {
            $this->logger->error('XML-RPC: ' . __METHOD__ . ': could not retrieve method signature nr.' . $sigNum . ' from remote server for method ' . $methodName);
            return false;
        }
    ...
    public function wrapXmlrpcServer($client, $extraOptions = array())
    {
        ...
        $response = $client->send($req, $timeout, $protocol);
        if ($response->faultCode()) {
            $this->logger->error('XML-RPC: ' . __METHOD__ . ': could not retrieve method list from remote server');
            ...
            if (!is_array($mList) || !count($mList)) {
                $this->logger->error('XML-RPC: ' . __METHOD__ . ': could not retrieve meaningful method list from remote server');
                return false;
            } else {
                ...
                foreach ($mList as $mName) {
                    if ($methodFilter == '' || preg_match($methodFilter, $mName)) {
                        ...
                        if ($methodWrap) {
                            ...
                            $this->logger->error('XML-RPC: ' . __METHOD__ . ': will not create class method to wrap remote method ' . $mName);
                        }
                    }
                }
                $source .= "}\n";
                if ($buildIt) {
                    ...
                    if ($allOK) {
                        ...
                        $this->logger->error('XML-RPC: ' . __METHOD__ . ': could not create class ' . $xmlrpcClassName . ' to wrap remote server ' . $client->server);
                        return false;
                    }
                } else {
